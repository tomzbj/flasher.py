...
class Cli:
    ...
    def UsageRaw(self):
        ...
        for i in range(block_count):
            if i % 64 == 0:
                print("%06X-%06X: " % (i * BLOCK_SIZE, (i + 64) * BLOCK_SIZE - 1), end="")
            if i == block_count - 1:
                print("R", end="")
    ...
    def __WriteFileMeta(self, fname, addr, size, timestamp, crc32):
        ...
        for i in range(64):
            ...
            for j in range(64):
                ...
                if meta_item[0] == 0:
                    meta_sector = meta_sector[:j * 64] + b'\xff' * 64 + meta_sector[(j + 1) * 64:]
                    i = j  # 记录最后一个空位置
    ...
    def DeleteFile(self):
        ...
        meta_sector = self.__Read(self.__GetMetaSectorAddr(), BLOCK_SIZE, False)
        for i in range(64):
            meta_item = meta_sector[i * 64:(i + 1) * 64]
            ...
            if fname == meta_fname.decode(encoding='utf-8'):
                self.__Write(b'\x00\x00', self.__GetMetaSectorAddr() + i * 64, False)
    ...
    def ListFile(self):
        ...
        file_list = self.__GetFileList()
        for fname, addr, size, timestr, crc32 in file_list: 
            print("%-20s  0x%08x  %-8d  %-20s  0x%08x" % (fname, addr, size, timestr, crc32))
            total_size += size
        print()
        block_lists = self.__FindAvailableSpace()
        available_blocks = 0
    ...
    def Check(self):
        ...
        file_list = self.__GetFileList()
        for fname, addr, size, timestr, crc32 in file_list:
            content = self.__Read(addr, size, False)
            if crc32 == zlib.crc32(content):
                print("%s crc32 ok!" % fname)
            else:
                print("%s crc32 mismatch!" % fname)
    ...
    def __FindAvailableSpace(self):
        ...
        for i in range(64):
            ...
            self.__SplitAvailableSpace(block_lists, (block_head, block_head + block_num))
